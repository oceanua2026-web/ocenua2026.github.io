...
const menuItems = [
  ...
  { id: "flashcards", label: "Флеш-карти", hint: "Класи, предмети, підручники, тести" },
  ...
  { id: "python", label: "Пітон", hint: "Програмування і тести" },
...
function renderMenuContent(sectionId) {
  ...
  const renderers = {
    ...
    about: () => renderStaticSection("Про програму", [
      "ОкеанUA - прототип карти, освітніх сторінок, флеш-карток і локальних чатів.",
      "Тут буде інформація про власника програми, адресу для зв'язку і пропозиція допомогти з картами міст, яких ще немає."
    ]),
...
function renderFlashcardsSection(
  ...
  const packageOptions = getNmtPackageOptions(gradeData);
  ...
  menuContent.innerHTML = `
    <div class="nmt-workspace school-flashcards-workspace">
      <div class="nmt-compact-head">
        ...
        ${subjects.length ? `
          <label class="nmt-subject-select">
            <span>Комплект</span>
            <select id="flashcardSubjectSelect">
              ${packageOptions.map((option) => `
                <option value="${escapeHtml(option.value)}"${option.subjectId === selectedFlashcardSubject && option.variantIndex === selectedFlashcardVariantIndex ? " selected" : ""}>
      ...
      ${gradeData.description ? `<p class="empty-text">${escapeHtml(gradeData.description)}</p>` : ""}
      ${subjects.length
  ...
  if (subjectSelect) {
    subjectSelect.addEventListener("change", () => {
      ...
      renderFlashcardsSection(selectedFlashcardGrade, selectedPackage.subjectId, 0, selectedPackage.variantIndex);
    });
  }
...
function renderChatSection() {
  ...
  menuContent.innerHTML = `
    ...
    <div class="pill-grid">
      <button type="button">Чат школи</button>
      <button type="button">Чат району</button>
      <button type="button">Чат громади</button>
      <button type="button">Чат міста</button>
    </div>
    <div class="linked-panel">
...
function renderStatsSection(sessionId = selectedStatsSessionId) {
  ...
  menuContent.innerHTML = `
    ...
    ` : `
      <p class="empty-text">Статистики ще немає. Відкрийте НМТ або Флеш-карти і виберіть відповідь у тестовій картці.</p>
    `}
  `;
...
function renderNmtOption(option, correctKey, response = null) {
  const correctKeys = String(correctKey || "").split(";").filter(Boolean);
  const selectedKeys = String(response?.selectedKey || "").split(";").filter(Boolean);
  const isCorrect = response && correctKeys.includes(option.key);
  const isWrong = response && selectedKeys.includes(option.key) && !correctKeys.includes(option.key);
  const classes = [isCorrect ? "is-correct" : "", isWrong ? "is-wrong" : ""].filter(Boolean).join(" ");
  return `
